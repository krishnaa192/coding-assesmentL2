...
const data= [
    ...
    const filterData = (filterItem) => {
        ...
        // Use data.length to iterate over the categories
        for (let i = 0; i < data.length; i++) {
            if (data[i].category_name == filterItem) {
                for (let j = 0; j < data[i].category_products.length; j++) {
                    // Created a new div that holds the product information
    ...
    // Call filterData function with "Men" as the default category
filterData("Men");
